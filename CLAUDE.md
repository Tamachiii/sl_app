...
| Student home (a QUEUE, not a week) | `student/StudentHome` (next-up card + "Then" + activity line), `lib/sessionQueue` (`buildQueue`/`buildDayStrip`/`flattenSessions` — pure), `student/SessionCard`, `hooks/useStudentProgramDetails`, `hooks/useSessionConfirmation`, `hooks/useMessages` (`useMyFeedbackSessionIds`). Next = first unconfirmed non-archived session in program order; a date never jumps the queue |
| Student sessions list | `student/StudentSessions` (a week heading appears ONLY when the coach labelled it — the label is a phase chip, not "Week N"), `student/SessionCard`, `hooks/useStudentProgramDetails`, `hooks/useMessages` (`useMyFeedbackSessionIds`) |
