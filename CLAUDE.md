...
| Student training surface (ONE page: was Home + Sessions) | `student/StudentTraining` (the block as a single ordered list — next session expanded at the top with the block position beside the program name, remaining work below it in program order, finished sessions under one DONE heading ordered by **real training date, newest first**, then past blocks as read-only history), `student/SessionCard` (`locked` suppresses the start affordance on past-block sessions), `lib/sessionQueue` (`buildQueue`/`flattenSessions` — pure), `hooks/useStudentProgramDetails` (`allPrograms: true`), `hooks/useSessionConfirmation`, `hooks/useMessages` (`useMyFeedbackSessionIds`). **No Mon–Sun day strip**: a 7-column grid with week arrows IS the calendar week. `/student/sessions` redirects to `/student`; the student nav is 3 tabs |
