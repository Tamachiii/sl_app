...
|---|---|---|
| Auth / login | `auth/LoginPage` (dark-first welcome — `STREETLIFT` brand tile + `Become / a big monster / like / Tony.` headline, blurred accent blob; **mobile**: single "Sign in" CTA reveals the email+password form in a step transition; **md+ (≥768px)**: 2-column split — hero on the left, form always visible on the right, no step; panel rendering is gated by a `useIsDesktop()` hook (`matchMedia('(min-width: 768px)')`) so only one "Sign In" button is in the DOM per viewport — tests rely on that; no register button; colors are inline CSS vars so the screen stays dark regardless of `.dark`), `hooks/useAuth`, `routes.jsx` | `lib/supabase` |
| Coach dashboard | `coach/CoachDashboard` (flat editorial h1 + `ui/UserMenu`; Students list with `W{n} · PROGRAM NAME` subtitle per student → `/coach/students/:id`; Recent Activity cards), `hooks/useStudents`, `hooks/useSessionConfirmation` (`useAllConfirmations`), `hooks/useProgram` (`useCoachDashboardPrograms` → per-student `{ programName, activeWeek }`) | `ui/UserMenu` |
...
| Coach session editor | `coach/SessionEditor` (editorial top bar + sl-pill actions; slot list is dnd-kit sortable via `DndContext` + `SortableContext` with `verticalListSortingStrategy`, reorder writes renumber `sort_order` 0..n-1), `coach/ExerciseSlotRow` (uses `useSortable({id: slot.id})` for drag handle — no up/down arrows; type pill removed; "Record" row is rectangular chips), `hooks/useSession`, `hooks/useExerciseLibrary`, `hooks/useDuplicate` | `ui/EditableText`, `@dnd-kit/core`, `@dnd-kit/sortable` |
...
- **Native `<dialog>` follows the OS `prefers-color-scheme`, not our `.dark` class.** The `ui/Dialog` primitive has explicit `bg-white text-gray-900` on the `<dialog>` element so it goes through the class-based dark remaps in `index.css` (`.dark .bg-white` → `ink-850`, `.dark .text-gray-900` → `ink-0`). Don't remove these classes — without them a user on a dark OS sees the dialog stay dark after flipping the app to light (and vice-versa).
- **iOS Safari auto-zooms focused `<input>`/`<textarea>`/`<select>` whose font-size is < 16px.** Every text-entry form element in the app is 16px for that reason — shared `inputCls`/`selectCls` constants (LoginPage, ExerciseSlotRow, ExerciseLibrary, SessionEditor, StudentGoalsSection, CopyDialog) plus inline per-element classes in MyGoals, SessionsFeed, CoachHome, ProgramSwitcher rename, SessionView confirm textarea, SlotCommentBox, EditableText, ExerciseProgressChart. Don't drop any of them below 16px. File-pickers / radios / checkboxes / sliders aren't affected.
- **`disabled:bg-*` Tailwind variants don't pick up `.dark .bg-*` remaps.** The variant generates `.disabled\:bg-gray-50:disabled` but our dark override is on `.dark .bg-gray-50` — different selector, so the disabled element stays light in dark mode. Use `disabled:bg-ink-100` (or another `ink-*` token) for disabled-state backgrounds inside anything that lives on a dark surface (dialogs, cards).
...
- **Coach nav tabs override NavLink's default active match.** `Students` and `Sessions` both live under the shared `/coach/student/:sid/…` prefix, so a plain `end: false` NavLink would light up BOTH tabs on any deep route. Instead each tab in `layout/navItems` declares a `matches(pathname)` predicate — Students matches `/coach/students*` or `/coach/student/:sid/week/…`, Sessions matches `/coach/sessions*` or `/coach/student/:sid/session/…/review`. `BottomNav`/`SideNav` call `useLocation()` and use `customActive ?? navLinkIsActive`.
- **Coach-section persistence across tab switches.** Two localStorage keys drive "pick up where you left off":
  - `sl_last_coach_students_path` — stores the full pathname (+ search) of the coach's deepest Students-section view (CoachHome, WeekView, or SessionEditor). Written by the `useRememberCoachStudentsPath()` hook (`src/hooks/useRememberCoachStudentsPath.jsx`), called unconditionally in each of those components. Read by `CoachHome`'s restore effect when the route is `/coach/students` with no `:studentId` — it validates the saved student is still in the students list, then `navigate(saved, { replace: true })`. Cleared when the coach explicitly picks "— Select a student —" from the dropdown (`handleSelect(null)`).
  - `sl_last_coach_session` — stores `JSON.stringify({studentId, sessionId})` for the last-open `SessionReview`. Read by `SessionsFeed`'s mount effect (redirects into the saved review). Cleared by the review's back button.
  - `sl_last_coach_sessions_student` — stores the student-id the coach last picked in `SessionsFeed`'s filter dropdown. Written by `handleFilterChange` (and removed when the coach picks "All students"). On mount, a dedicated effect copies the saved id back into the `?student=` search param when the URL has none, so the filter survives tab switches. The filter itself still lives in the URL — localStorage just repopulates it on return to `/coach/sessions`.
- **Back buttons in coach deep views navigate to logical parents, not `navigate(-1)`.** Because the tab-restore `navigate(saved, { replace: true })` pollutes browser history (the previous entry is whichever tab the coach came through), `navigate(-1)` can land somewhere unexpected. `WeekView` back → `/coach/students/:studentId`; `SessionEditor` back → `/coach/student/:studentId/week/:weekId`; `SessionReview` back → `/coach/sessions`. `WeekView`'s `deleteWeek` success handler navigates the same way.
