...
| Adopt swap → program edit (Phase 3.1) | `coach/SessionReview` ("Adopt into program" on the swap banner) + `coach/AdoptSwapDialog` (blast-radius confirm), `hooks/useAdoptSwap` (`useAdoptSwap` + `useAdoptSwapPreview` dry-run), migration `2026_07_18_adopt_swap.sql` (`adopt_swap` SECURITY DEFINER RPC — coach self-auth, **forward-only**: flips `exercise_id` X→Y on upcoming unconfirmed occurrences in the same program, preserves the reviewed session's history, keeps `target_*` as a seed, notifies the student via `swap_adopted`). Coach-only, online-only, swap-only |
