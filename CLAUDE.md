...
| Coach session review | `coach/SessionReview`, `coach/SessionFeedbackComposer`, `coach/SessionFeedbackSent`, `hooks/useSession`, `hooks/useSetLogs`, `hooks/useSlotComments`, `hooks/useSessionConfirmation`, `hooks/useMessages` (`useSessionFeedback`) |
...
- **Coach session feedback is one-shot per session.** A coach can submit feedback at most once for any given session. Enforced DB-side by a UNIQUE partial index on `messages(session_id) WHERE session_id IS NOT NULL` (paired with the RLS rule that only the session's coach can write a row with `session_id` set). UI-side, [SessionReview.jsx](src/components/coach/SessionReview.jsx) calls `useSessionFeedback(sessionId)` from [hooks/useMessages.js](src/hooks/useMessages.js) and renders [SessionFeedbackSent.jsx](src/components/coach/SessionFeedbackSent.jsx) (read-only body + "open thread" link) when a feedback row exists, otherwise falls through to [SessionFeedbackComposer.jsx](src/components/coach/SessionFeedbackComposer.jsx). Realtime invalidation on `[messages]` swaps the composer for the read-only card automatically if a row appears mid-page. Editing is intentionally **not** supported — the messages UPDATE trigger pins `body` and `session_id`, so feedback is immutable once sent.
