...
src/
  ...
  components/
    ...
    coach/       CoachHome (Athletes roster + single-student tabs)  StudentWeekStrip  ProgramSwitcher
                 ...
                 StudentHeader  StudentProgrammingSection  StudentProgressSection
...
| Coach single-student view (2 tabs + header) | `coach/CoachHome`, `coach/StudentHeader` (avatar/name/coaching-since + View sessions + Message — always visible above the strip), `coach/StudentProgrammingSection`, `coach/StudentProgressSection` (composes `StudentGoalsSection` then `StudentStatsSection`), `routes.jsx`. Profile is **not** a tab (dissolved into the header); `profile`/`messaging` redirect to `programming`, `goals`/`stats` redirect to `progress` |
