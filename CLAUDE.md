...
| Student program authoring (Phase 3.4, online-only) | `student/StudentProgramAuthor` (lean self-contained draft builder — weeks/sessions/slots + slot targets; English-only; gates on `useOnlineStatus`), `hooks/useAuthoring` (`useMyDraft`/`useDraftTree` + create/add/update/delete/submit/discard mutations — plain server-id INSERTs, **no set_logs**, all in the offline-safety `ONLINE_ONLY` list), route `/student/author` + CTA on `student/StudentProfile`. DB gate + `approve_program` in `2026_07_21_student_program_authoring.sql` (see the Phase 3.4 INVARIANTS entry) |
...
| Personal records + bodyweight | `student/PersonalRecords`, `student/BodyweightCard`, `hooks/useStudentRecords`, `hooks/useBodyweight`, `lib/records.js` (Epley e1RM + `systemLoad`), migration `2026_07_14_bodyweight_logs.sql` |
