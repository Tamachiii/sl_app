...
A student can have many programs (periodization blocks) at once, but only one is **active** at a time (partial unique index `programs_one_active_per_student`). Students only ever see the active program on Home / Sessions — previous blocks stay invisible in those lists so the active block drives training context. On the student **Stats** calendar, dated sessions from previous (non-active) programs still render as a **muted history dot** (fed by `useStudentHistoricalSessions`, merged into the calendar-only payload) so students can peek at when they trained in prior blocks and tap through to a read-only view of that past session. Aggregates (weekly volume, exercise progression, sessions/sets/RPE) stay scoped to the active block. Coaches see a **ProgramSwitcher** dropdown at the top of the Program card on `CoachHome`: the trigger shows the currently-selected program (name + `ACTIVE` badge + week count); clicking opens a listbox popover of all blocks with per-row drag handles for vertical reorder (dnd-kit). Sibling `+` and `⋮` icon buttons add a new block or open the Manage dialog (rename / set-active / delete-with-confirmation) for the selected block. The whole thing — program selector, a hairline divider, and the week timeline — lives inside a single `sl-card` (`ProgramCard` in `CoachHome.jsx`), so weeks visually belong to the selected program. When a program has no weeks, the bottom row collapses to a single full-width dashed `+ WEEK` button (no separate "empty state" message). Selected program is URL-synced as `?program=<id>`; the default is the active one. Deleting the *active* program is blocked while other programs exist — coaches must activate another first. Migration: `2026_04_23_programs_crud.sql` (adds `sort_order` + `is_active` with chronological backfill).
