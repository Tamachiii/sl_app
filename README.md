...
**Student**
1. Logs in → **Training tab** (`/student`) — one page, one list. A greeting line states what they did ("last trained 5 days ago", or the week's rhythm), then the block: the next session expanded at the top with Start and their place in it ("Session 6 of 7"), the remaining sessions below it, then everything finished under a **Done** heading ordered by the day it was really trained. Past blocks follow, each collapsed to a single row ("Past program · 15 sessions") that opens as read-only history. Miss a Sunday and it is still simply next — nothing to re-date. There is deliberately no Mon–Sun day strip: the month calendar in Stats is where "when did I train" belongs.
