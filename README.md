...
**Student**
1. Logs in → **Home tab** (`/student`) — a **queue**, not a schedule. A greeting line states what you did ("last trained 3 days ago · 2 sessions in the last 7 days"), then a 7-day strip showing the days you actually trained (with coach recommendations drawn dashed behind them), then an always-expanded **Next session** card with a Start CTA and your place in the block ("Session 7 of 24"), followed by the next couple in line. Next is simply the next unfinished session — miss a Sunday and it is still next, with nothing to re-date.
2. **Sessions tab** (`/student/sessions`) — the full program; a heading appears only where the coach named a phase ("Deload", "Peak"). Accordion session cards (one open at a time); tap "Start session" to open `SessionView`. Each expanded exercise shows a **"Last time" hint** — the sets you performed for that lift in your most recent prior session (e.g. "3 × 8 @ 100kg · 5 days ago") — so progressive overload has a target to beat.
...
- **Student-side feedback surface**: feedback shows up in two places on the student side. `SessionView` reads `useSessionFeedback(sessionId)` and renders a "Coach's feedback" card between the progress bar and the slot groups when a feedback row exists — this is what the deep-link from a feedback message lands on. `SessionCard` shows a small "feedback" pill in its trailing block when `hasFeedback` is true; `StudentHome` and `StudentSessions` source that flag from `useMyFeedbackSessionIds()` (a single batched query for the student's feedback rows), so a student can tell at a glance which sessions have a follow-up to read.
